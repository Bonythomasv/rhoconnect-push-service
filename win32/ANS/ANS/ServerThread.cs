...
namespace ANS
{
    ...
    public class ServerThread
    {
        ...
        private MessageQueue initQueue(string name)
        {
        ...
        public ServerThread(Settings _settings, int _serverIndex)
        {
            ...
            queues = settings.Servers[serverIndex].Queues.ToDictionary(q => q.GetKey(), q => initQueue(".\\Private$\\" + q.GetKey()));
        ...
        public void Register(AppInfo ai)
        {
            ...
            lock (thisLock)
            {
                ...
                else
                {
                    ...
                    queues.Add(key, initQueue(".\\Private$\\" + key));
        ...
        public void LoadQueues()
        {
            lock (thisLock)
            {
                ...
                {
                    queues.Add(ai.GetKey(), initQueue(".\\Private$\\" + ai.GetKey()));
                }
            }
        }
        ...
        private void Send(MessageQueue queue, OutgoingMessage om)
        {
        ...
        private void Run()
        {
            ...
            while (true)
            {
                ...
                else
                {
                    ...
                    lock (thisLock)
                    {
                        try
                        {
                            ...
                            if(queues.ContainsKey(ai.GetKey())) {
                                MessageQueue q = queues[ai.GetKey()];
                                currentReaders = !q.CanRead;
                                path = ai.Path;
                                args = ai.Args;
                            }
                            else {
                    ...
                    if(currentReaders == false) {
                        Process.Start(path, args);
                        while(currentReaders == false) {
                            Thread.Sleep(10000);
                            lock (thisLock) {
                                try
                                {
